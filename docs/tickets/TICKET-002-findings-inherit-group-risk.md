...
def inherit_group_risk_to_findings(
    ...
    *,
    ...
    """Setzt ``Finding.risk_band``/``action_type`` auf den Wert der zugeordneten ApplicationGroup.
    ...
    stmt = (
        ...
        .where(
            ...
            | (Finding.action_type.is_distinct_from(ApplicationGroup.action_type))
            | (Finding.risk_band_source.is_distinct_from("llm_group"))
        )
        .values(
            risk_band=ApplicationGroup.risk_band,
            action_type=ApplicationGroup.action_type,
            risk_band_source="llm_group",
...
Schema-Check vorab: existiert ``risk_band_source`` als String-Spalte auf ``Finding``? Wenn ja, wird ``"llm_group"`` als neuer Enum-Wert zugelassen (kein DB-Constraint-Update noetig wenn Spalte einfach String). Wenn der bestehende Constraint ``risk_band_source IN ('pretriage', 'manual')`` definiert, **Migration noetig** um ``'llm_group'`` zu ergaenzen.
...
1. Happy: Finding mit Group, Group hat ``risk_band='act'`` → Finding bekommt ``risk_band='act'``, ``action_type='patch'``, ``risk_band_source='llm_group'``.
...
4. Finding hat schon korrekten ``risk_band`` (idempotent) → kein UPDATE.
...
Erwartung (a): viele Rows mit ``f.risk_band='pending'`` + ``group_band='act'`` o.ae. — das ist die Vererbungs-Luecke. Erwartung (b): ``risk_band_source`` ist freier String (keine Check-Constraint die ``'llm_group'`` blockt).
