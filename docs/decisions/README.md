...
| [0061](0061-fix-ownership-lang-pkgs-upstream-lane.md) | Fix-Ownership: lang-pkgs-Fixes sind nicht host-applizierbar — dritte Fix-Lane `upstream` (kein `act`), deterministisch aus `finding_class`; behebt falsche „Apply app update"-Empfehlung für gobinary/stdlib-Findings (z. B. tailscaled). Single-Source `fix_lane_for`. Tier 0, erweitert ADR-0053 | Akzeptiert (Umsetzung offen — Block AG) |
| [0062](0062-agent-host-update-availability.md) | Host-Agent löst Binary→Paket auf (`rpm -qf`/`dpkg -S`) und meldet `host_update_available` (`dnf check-update`/`apt -s upgrade`, read-only, kein EOL-Reasoning); promotet `upstream`→`patch` wenn das Paket wirklich updatebar ist. Tier 1, verfeinert ADR-0061 | Akzeptiert (Umsetzung offen — Block AH) |
| [0063](0063-agentic-upstream-update-search.md) | Optionale, operator-gated, **beratende** agentische Upstream-Update-Suche (GitHub/Vendor) für nicht-paketierte/EOL-Fälle; Seed aus Trivy-Buildinfo; nie Auto-Band-Flip, air-gap-default-aus. Tier 2, kein Chat (ADR-0050 bleibt) | Akzeptiert (Umsetzung offen — Block AI) |
