...
**Block AF implementiert 2026-06-12 — Getrennte Modelle für Risk-Reviewer und Per-Group-Chat (ADR-0057), uncommitted.** Branch `feat/block-af-reviewer-chat-model-split` (von `main`), Zielversion v0.21.0. **6 Phasen durch, reviewer GRÜN (DoD 1–7 PASS), security-auditor APPROVED** (alle 7 Fokuspunkte grün — kein Key-Leak in der Test-Connection-Response, Auth/Rate-Limit/CSRF/Scope korrekt). Quality-Gates: `ruff`/`ruff format`/`mypy app/` grün, Default-`pytest` **2554 passed / 187 skipped / 626 deselected** (~27 s, +62 neue Pure-Unit-Tests: forms 10 · update 8 · test_connection 6 · llm_client +5 · group_chat +4 · worker-fingerprint +3 · model-split-introspektion 4). Sprach-Sweep grün. Phasen: P1 Schema+Migration `0024`, P2 Verdrahtung (`llm_client.model_override`/`llm_worker`/`group_chat`/`settings.py`), P3 Form+View+Test-Conn-Doppelprobe, P4 Frontend (2 Felder + 2-Zeilen-Test-Result), P5 Tests, P6 Doku. **Security-GELB (pre-existing, NICHT von AF eingeführt):** `llm_client.py:280` loggt rohen Exception-Text (`error=err`), structlog-Redaction matcht das Feld `error` nicht → TD-018, separat zu fixen, nicht block-blockierend (AF hat den Response-Pfad sogar gehärtet: Error-Code-Mapping statt Roh-Durchreichung). **Beim User anstehend (db_integration/E2E/Browser, nicht proaktiv):** Alembic-Roundtrip `0024` (Rename + Add + Backfill + Downgrade), Migration-Backfill gegen echtes Postgres, Lauf `tests/integration/test_llm_settings_view_db.py` (Feld-Renames + 2-Teil-Shape bereits eingepflegt), Live-Test-Connection-Doppelprobe gegen DeepInfra (verifiziert den `DeepSeek-V4-Flash`-Literal), Operator-Browser-Smoke (Provider-Tab zwei Modell-Felder, Preset füllt beide, Test-Result zwei Zeilen, Chat nutzt Chat-Modell). **Kein Commit ohne explizite User-Anweisung** ([No-Auto-Commit]); **Tag erst nach Merge auf main** ([Tag-only-on-main-after-Merge]). Spec: `docs/blocks/AF-reviewer-chat-model-split.md`, ADR: `docs/decisions/0057-separate-reviewer-and-chat-models.md`. _(Original-Planungsnotiz unten erhalten.)_

**Block AF (Planungsnotiz) — Getrennte Modelle für Risk-Reviewer und Per-Group-Chat (ADR-0057).** Branch `feat/block-af-reviewer-chat-model-split` (von `main`), Zielversion v0.21.0. Trennt das geteilte `Setting.llm_model` in **`llm_reviewer_model`** (Rename, Default `openai/gpt-oss-120b`, Konsument `llm_worker.py` Pass 1+2) und **`llm_chat_model`** (neu, Default `deepseek-ai/DeepSeek-V4-Flash`, Konsument `group_chat.py`). **Ein** Provider geteilt (`llm_base_url`/`llm_api_key_encrypted`) — *ein Provider, zwei Modelle*. **User-Entscheidungen 2026-06-12:** (1) explizit umbenennen statt additiv; (2) `llm_chat_model` per Migration `0024` **forced** auf `DeepSeek-V4-Flash` für alle Zeilen (404-Risiko bei Providern die das Modell nicht hosten — bewusst akzeptiert, Test-Connection deckt es auf); (3) Test-Connection probt **beide** Modelle einzeln. Migration `0024` (`down_revision=0023_block_ae_group_chat`): Rename `llm_model`→`llm_reviewer_model` + Add `llm_chat_model` + Backfill; Downgrade kehrt um. Touchpoints: `models.py`, `forms.py`, `llm_settings.py` (View+Presets+Audit+Test-Conn), `llm_provider.html`+`llm_settings.js` (2. Feld), `llm_client.build_client_from_settings` (`model_override`), `group_chat.py` (Chat-Modell+Snapshot), `llm_worker.py` (Feldname), `settings.py::active_model`. Pass 1 und Pass 2 teilen weiter **ein** Reviewer-Modell. Spec: `docs/blocks/AF-reviewer-chat-model-split.md`, ADR: `docs/decisions/0057-separate-reviewer-and-chat-models.md`. _(Diese Planungsnotiz ist mit der Implementierung oben umgesetzt.)_

**Block AE implementiert 2026-06-11 — Per-Group AI-Chat (ADR-0055), uncommitted.** Branch `feat/block-ae-group-chat` (von `main`). Wieder-Einführung eines LLM-Chats, **fokussiert pro (Server, Application-Group)** — nutzt den Re-Open-Trigger von ADR-0050 (server-weiter Chat bleibt verworfen). Einstieg: **„Help"-Button pro Group-Row** in den Operator-Workflows (`_action_needed_section.html`), nirgends sonst. **Vier bestätigte Entscheidungen umgesetzt:** (1) eine Konversation pro (Server, Group), DB-persistiert, `UNIQUE(server_id, application_group_id)`, kein Archiv, „New Chat" = CASCADE-Delete; (2) SSE-Streaming via `LlmClient.stream_chat` (erhalten); (3) Kontext-**Snapshot** bei Chat-Start (Host-Fingerprint + Services + Listener + alle OPEN-Findings der Group, in den persistierten System-Prompt gerendert, `findings_snapshot_at`); (4) **kein Token-Cap** (Chat ruft `llm_budget` nicht).

- **Phase 1 — Schema/Migration/ORM** (`backend-implementer`). Migration `0023_block_ae_group_chat` (`down_revision=0022_fix_lane_evaluation`): Enum `chat_message_role` + `group_chat_conversations` (UNIQUE `server_id,application_group_id`) + `group_chat_messages` (INDEX `conversation_id,created_at,id`), `downgrade` child→parent + Enum `checkfirst=False`. ORM `GroupChatConversation`/`GroupChatMessage`/`ChatMessageRole` (StrEnum, `messages` cascade `all, delete-orphan` + `passive_deletes`). **Implementer-Entscheidung:** `application_group_id` ist **BigInteger** (nicht `Integer` wie wörtlich in §3) — `application_groups.id` ist BigInteger, Integer→BigInteger-FK wäre Typ-Mismatch; `server_id` bleibt Integer. **conftest carve-out** `_PURE_UNIT_OVERRIDES={"tests/models/test_group_chat.py"}` (nötig, da `tests/models/` pauschal als db_integration gesweept wird).
- **Phase 2 — Prompt-Builder** (`backend-implementer`). `app/services/group_chat_prompt.py`: `CHAT_SUGGESTIONS=["Explain attack vector"]` (single-source), `build_group_system_prompt(...)` (keyword-only, reiner String-Builder, **keine** DB-Query). `_safe` portiert (Control/NUL-Strip + Längen-Cap) **plus** Marker-Neutralisierung via Zero-Width-Space (Block-AE-Härtung: eingebettete `<<TRIVY_DATA_END>>` in untrusted Scanner-Strings können den Daten-Block nicht schließen).
- **Phase 3 — Blueprint + SSE** (`backend-implementer`). `app/api/group_chat.py` (`group_chat_bp`, registriert in `__init__.py`): `GET /chat` (Sub-View, `120/min`), `POST /messages` (Lazy-Create + Snapshot, `30/min`, 8KB-Cap, `400 llm_not_configured`), `GET /stream` (SSE, `60/h`, Assistant+Usage in frischer Session nach Stream-Ende, generischer `provider_error`-Frame ohne Key-Leak, **kein `llm_budget`**), `POST /new` (CASCADE-Delete). Gemeinsamer `_guard_or_404` (Server aktiv **und** Group hat OPEN-Findings auf diesem Server → IDOR-Schutz). Importiert `_load_host_snapshot`/`_load_application_groups_for_server` aus `server_detail.py`. **Design-Klärung:** Chat ist per-(Server,Group), nicht per-Lane (ADR §1 UNIQUE-Key ohne fix_lane); Snapshot enthält alle Group-Findings, Lane-Headline ist deterministische Primary-Lane.
- **Phase 4 — Frontend** (`frontend-implementer`). Help-Button (4. Spalte `workflow-table__ask`, `sd-ask-btn`, ChatGlyph) in `_action_needed_section.html`, `hx-get`→`group_chat.show`, Target `#detail-pane` + `_partial_shell.html`-Wrapper (Settings-Sub-View-Muster, **nicht** `#detail-pane-content` wie ursprünglich im Prompt). Sub-View `servers/group_chat.html` + Single-Source-Partial `_partials/group_chat_message.html` (ID `chat-msg-<id>`). `frontend/src/js/group_chat.js` (Alpine + EventSource, Deltas via `textContent`, `bubble_html` server-escaped via `insertAdjacentHTML`, Enter/Shift+Enter/Esc, Autoscroll). CSS-Port `sd-chat-*`/`sd-ask-btn`/`sd-newchat`/`workflow-table__ask` token-only. Frontend-Build grün. Ripple: `test_action_needed_drift_hint.py`/`test_action_needed_drilldown.py` bekommen `server=` + 4-Spalten-Count.
- **Phase 5 — Review.** `reviewer` **GRÜN** (alle 9 DoD-Punkte PASS). `security-auditor` **APPROVED** (kein ROT; eine GELB-Härtung — `GET /chat` ohne Limit — **direkt gefixt**: `@limiter.limit("120/minute")`). Marker-Escape (inkl. NUL-Split + Truncation-Reform) + IDOR adversarial gegengeprüft.

**Quality-Gates:** `ruff check`/`ruff format --check`/`mypy app/` grün; Default-`pytest` **2490 passed / 186 skipped / 626 deselected** (~24 s); +69 neue Block-AE-Pure-Unit/Adversarial-Tests (models 11 · prompt 31 · api 14 · template/xss 13). Sprach-Sweep grün. **3 Failures `test_settings_subpages_smoke.py::…[settings/servers.html…]` (`delete_findings_form undefined`) sind block-AE-fremd und pre-existing** (per Stash-Vergleich verifiziert — TD-Kandidat). **Beim User anstehend (db_integration/E2E/Browser, nicht proaktiv):** Alembic-Roundtrip `0023`, UNIQUE/CASCADE-Semantik, SSE-E2E gegen Live-Provider, Operator-Browser-Smoke (Help→Swap, Stream-Tippen, New-Chat-Wipe, Esc/Enter). **Doku:** ARCHITECTURE.md + CLAUDE.md §17 + CHANGELOG aktualisiert, ADR-0055 im Index. **Kein Commit ohne explizite User-Anweisung** ([No-Auto-Commit]); **Tag erst nach Merge auf main** ([Tag-only-on-main-after-Merge]).

**Aktiv 2026-06-10 — TICKET-010 „Jetzt"-Konsistenz umgesetzt (ADR-0052), uncommitted.** Branch `feat/ticket-010-live-now` (von `main`, nach Merge von `feat/remove-ai-assessment`). Vier Etappen + ADR, alle implementiert, Reviewer **APPROVED (0 ROT)**. **Kein Schema, keine Migration, kein neuer Endpoint.** (1) **Reopen-on-Redetect:** neue Phase „1c" in `findings_ingest.py` vor dem Upsert — RESOLVED-Findings deren `(identifier_key, package_name)` im aktuellen Scan liegt → `status='open', resolved_at=NULL`; **ACK bleibt ACK** (User-Entscheidung 1); `findings_reopened` durch `ScanIngestResult` → `ScanProcessingResult` → `scan.ingested`-Audit → `result_to_jsonb`. (2) **Pass-2 OPEN-only:** beide `_do_pass2`-Loads filtern `status='open'` (identische Fingerprint-Domäne wie `pass2_enqueue` → Dauer-Re-Enqueue tot); Race-Guard: alle Findings zwischen Pickup und LLM-Call geschlossen → Job `skipped` ohne Call; einmaliger Cache-Miss-Burst pro betroffener (group, server). (3) **Live-Worst-Finding:** Loader-Query 4 als `DISTINCT ON (application_group_id)` über offene Findings mit §15-Order (`_severity_rank_expr` aus `findings_query` wiederverwendet), neuer Entry-Key `worst_finding_drift`; Template-Gate nur noch `{% if worst_finding %}`, Drift-Hint **„re-evaluation pending"** (Entscheidung 2) in `_action_needed_section.html` + `application_group_card.html`, token-only CSS. (4) **Sofort-Re-Eval bei Triage** (Entscheidung 3): `Pass2Trigger` + `"triage_action"`; Enqueue nach Status-Write in `acknowledge`/`reopen`/`group_acknowledge`/`bulk_acknowledge` (`findings.py`) + `api/bulk.py` Flavors A/B/C, je 0-geändert-Gate, vor `commit()`, distinct Server bei Bulk. **+~80 neue Pure-Unit-Tests** (Reopen-Statement-Level, Bug-B-Fingerprint-Regression + No-Re-Enqueue-Konvergenz, Drift-Matrix + DISTINCT-ON-Statement-Inspektion, Trigger-Gates alle Endpoints). Default-`pytest` **2329 passed / 186 skipped / 627 deselected** (~26 s), `ruff`/`mypy app/` grün (`ruff format`-Befund `tests/adversarial/test_agent_files_path_traversal.py` ist vorbestehend auf main, nicht im Ticket-Diff). **Beim User anstehend:** db_integration (Upsert-/Reopen-Semantik, `DISTINCT ON`, angepasstes `tests/integration/test_application_group_cards_db.py`), Operator-Browser-Smoke (ftp-server-Card zeigt offenes Worst-Finding, /findings ↔ Detail konsistent, Diagnose-Query 7 leer nach erstem Scan-Zyklus), Entscheidung ob `diagnose_cve-2026-31431.sql` (untracked, von ADR-0052 referenziert) eingecheckt wird. Frontend-Build für die neue Hint-CSS-Klasse läuft beim nächsten Docker-Build. **Kein Commit ohne explizite User-Anweisung** ([No-Auto-Commit]). Hinweis GELB (Folge-Kandidat): `_severity_rank_expr`-Private-Import in `server_detail.py` ggf. als öffentliches Re-Export aufräumen; `_partials/application_group_card.html` hat keinen Production-Konsumenten mehr (orphaned-Partial-TD-Kandidat).

**Abgeschlossen 2026-06-07 — "Request AI Assessment"-Chat-Feature entfernt (ADR-0050).** Branch `feat/remove-ai-assessment` (von `main`). Das server-weite interaktive LLM-Chat-Assessment (Block G) ist **ersatzlos entfernt** — UI/Routes/Prompts/Chat-Services/JS/3 DB-Tabellen/Tests. User-Entscheidung: **voll entfernen inkl. Drop-Migration `0017_remove_llm_chat`** (down() rekonstruiert die drei Tabellen + zwei Enums leer). Geteilte Provider-Config + Risk-Reviewer (Block P) **unangetastet**. Entfernt: `app/api/llm_chat.py`, `app/services/{llm_prompt,llm_update_hook,llm_token_tracker}.py`, `app/templates/chat/`, `app/static/js/llm_chat.js`, der Detail-Button + `sd-ai-button`-CSS, Models `LlmConversation`/`LlmMessage`/`LlmConversationFinding` + Enums, `_archive_active_conversations()` + Confirm-Modal/Flash in `llm_settings.*`, der Scan-Hook in `scan_processing.py`, Context-Processor `_inject_llm_configured`, Audit-Action `llm.queried` aus `KNOWN_ACTIONS`, 6 Chat-Test-Dateien. Behalten: `llm_client.py`, `llm_budget.py`, `/settings/llm`-Provider-Tab, Audit-Event `llm.provider_changed` (ohne `archived_conversations`). Default-`pytest` **2233 passed / 794 skipped / 5 deselected** (~26 s; hohe Skip-Zahl weil lokal kein Postgres läuft — alle db_integration skippen statt deselected), `ruff`/`ruff format`/`mypy app/` grün. ADR-0050 + CHANGELOG + README-Index + Status-Notes auf ADR-0023/0026 geschrieben. **`ARCHITECTURE.md` synchronisiert** (nach dem parallelen Rewrite): In-Scope-Liste, Datenmodell-`llm_*`-Bullet, HTMX-Polling-SSE-Satz, Server-Detail-`/chat`-Route, DoS-LLM-Kostenschutz, §11-Chat-Abschnitt, `audit_events`-Liste (`llm.queried`→`llm.provider_changed`). **Alembic-Roundtrip `0017` steht beim User an** (db_integration). **Operator-Browser-Smoke offen:** Server-Detail-Header ohne AI-Button, `/settings/llm` ohne Conversation-Count/Confirm-Modal, Provider-Switch ohne Archivierungs-Flash. **Kein Commit ohne explizite User-Anweisung** ([No-Auto-Commit]). **Tag erst nach Merge auf main** ([Tag-only-on-main-after-Merge]).

**Block AD abgeschlossen 2026-06-04 — Settings-Redesign: horizontale Tab-Nav + `s-*`-Schicht (ADR-0047).** Branch `feat/block-ad-settings-redesign` (von `main` abgezweigt, nachdem die Mockup-Design-Dateien `docs/design/*` als eigener Commit `eff172f` auf `main` lagen — User-Entscheidung). Zielversion v0.19.0. **Reines Restyling — keine Routen-/Schema-/Render-Helper-Änderung, keine Migration, kein neuer Endpoint.** `app/views/_settings_shell.py` unangetastet (DoD #8 grün); `settings.py`/`llm_settings.py` im Block selbst unverändert, nur im Folge-Fix minimal angefasst (Feeds-Move About, User-OK). Default-`pytest` **2253 passed / 201 skipped / 658 deselected** (~25 s; +36 neue Template-Tests, −5 obsolete Legacy-Smokes), `ruff`/`ruff format`/`mypy app/` grün, Frontend-Build grün (CSS-Bundle ohne `sd-manage`). Spec: `docs/blocks/AD-settings-redesign.md`, Prompt: `docs/blocks/AD-prompt.md`.

- **Phase 0 — ADR + Branch (`e27d52f`).** ADR-0047 (löst Sekundär-Nav-Teil von ADR-0016 ab), Index-Eintrag, ADR-0016-Status-Note, Block-Spec/-Prompt committet.
- **Phase A — CSS-Port (`65254a9`).** `docs/design/settings.css` → `frontend/src/css/components/settings.css` (1:1, Token-only). Zwei Eingriffe: Garbage-Kommentar Z865 gefixt, `.profile-menu__item--active` nach `profile-menu.css`. `@import` vor legacy-shim. Anchor-Tabs: `text-decoration:none` ergänzt (App nutzt `<a>` statt Mockup-`<button>`).
- **Phase B — Shell-Umbau (`59bb026`).** `_nav.html` vertikale `w-56`-Menu → `<nav class="settings-tabs">` mit 7 Anchor-Tabs (HTMX 1:1). `_shell.html`/`_page.html`: Tabs oben + `#settings-content.settings` (Frame bleibt über Tab-Swap stehen), `#detail-pane-content` unverändert. `_profile_dropdown.html`: Settings-active-Marker (Pfad-basiert).
- **Phase C1 — servers/tags/groups (`8b1f580`).** `s-table`/`s-pill`/`s-overflow`/`s-tags__*`/`s-groups__*`/`s-empty`. Alle Forms/CSRF/Confirms/Reorder-Endpoints unverändert. Tag-Color als Alpine-Palette-Popover + nativer Picker. Tags-/groups-Template-Tests auf `s-*`-Selektoren umgestellt.
- **Phase C2 — llm_provider/llm_reviewer/llm_debug_log/master_key/about (`f1f2008`).** `s-card`/`s-fields-grid`/`s-feeds`/`s-statusbar`/`s-slider-row`/`s-kpis`/`s-twoup`/`s-kv`/`s-subtabs`/`s-log`/`s-key-status`/`s-warning`/`s-key-reveal`/`s-about-grid`. `llmProviderForm`-Alpine-API unverändert. Vier DaisyUI-Modals → token-gestylte Alpine-`x-show`-Overlays (Provider-Switch, Reviewer-Mode/-Requeue/-Concurrency); Master-Key-Rotate-Confirm → nativer `confirm()`.
- **Phase D — Tests (`f2debf5`).** Neu: `test_settings_nav.py` (7 Tabs, Reihenfolge, Active-Marker, HTMX-Vertrag, Badge), `test_settings_subpages_smoke.py` (Render-Smoke + s-*-Indikator + token-level DaisyUI-Absence + Header-Pattern pro Subseite). `test_settings_legacy_still_renders.py` gelöscht.
- **Phase E — Cleanup + Doku.** `settings-manage.css` gelöscht + `@import` raus (DoD #6 `sd-manage`-grep clean). ARCHITECTURE.md §Settings-UI-Aufbau, CHANGELOG v0.19.0, ADR-Index (Phase 0), TD-016 (Tag-Usage-Zählung) + TD-017 (Debug-Log Filter/Pause/Copy/Live-Stream), STATE.md.
- **Folge-Fixes (nach erstem Sichttest, ADR-0047 §Nachtrag).** (1) `settings_tabs.js` zieht den Tab-Active-Marker client-seitig nach (Tab-Leiste liegt außerhalb des HTMX-Swap-Targets — vorher wechselte der Cyan-Border erst beim Full-Reload). (2) Modals (Provider-Switch + 3 Reviewer-Modals) per `x-teleport="body"` viewport-zentriert (vorher oben-links wegen `container-type:inline-size` auf `.settings`). (3) External Feeds (EPSS/CISA-KEV) vom LLM-Provider- auf den About-Tab verschoben (`about_view` reicht `feed_statuses` durch — minimale View-Änderung mit User-OK). (4) Master-Key-Tab-Badge entfernt. Default-`pytest` **2254 passed**.

**Bewusst weggelassen (ADR-0047 §Out of Scope / Mockup ohne Backend):** Debug-Log-Filter/Level/Pause/Copy/Live-Stream (TD-017), Tag-Usage-Zählung (TD-016, View liefert keine Zahl — Platzhalter „—"), Eyebrow-Nummerierung „01 / 07" (User-Entscheidung), `servers/settings.html` (andere Seite), Topbar/Sidebar/Footer. **DoD-Hinweis:** Die literalen DoD-#5-greps (`card\b|btn\b|badge\b|alert\b|tab-active`) treffen die neue `s-*`-Namespace (`s-btn`/`s-card`/`s-overflow__btn`/`settings-tabs__badge`/`role="alert"`) und sind als wörtliche „0 Treffer"-Checks nicht erfüllbar — die Absicht (keine **bare** DaisyUI-Komponentenklassen) ist via Token-Level-Sweep in `test_settings_subpages_smoke.py` verifiziert (0 Treffer). **Operator-Browser-Smoke offen** (vom User abzuhaken): Tab-Wechsel-Swap + URL-Push, Direkt-URL `/settings/tags` (Vollseite), Refresh auf Sub-Tab, Profile-Dropdown→Settings (Detail-Pane-Fragment), Master-Key-Rotate-Reveal, Tag-Farbwechsel, Gruppen-Reorder, schmales Fenster (Container-Query-Fallbacks), Reviewer-Modals (Mode/Requeue/Concurrency mit Master-Key). **Tag `v0.19.0`** erst nach Branch-Merge auf main ([Tag-only-on-main-after-Merge]). **Kein Merge, kein Tag durch mich** (Block-Auftrag).

**Block AC abgeschlossen 2026-06-04 — Sidebar Group State (ADR-0046).** Aufgeklappte Sidebar-Gruppen bleiben persistent offen (Polling-Swap, Reload, Browser-Sessions): Cookie `sidebar_open_groups` (JS schreibt aus DOM-Ist-Zustand), Server liest in `build_sidebar_context()` und rendert `open` direkt — kein Schema, kein Endpoint, kein localStorage. Branch `feat/block-ac-sidebar-group-state` (von aktualisiertem `main` abgezweigt — Block AB war bereits gemergt, kein `sidebar.js`-Konflikt), Zielversion v0.18.0, drei Phasen. Default-`pytest` **2222 passed / 201 skipped / 658 deselected** (~27 s; +18 neue Pure-Unit-Tests in `tests/views/test_sidebar_open_groups.py`), `ruff`/`ruff format`/`mypy app/` grün. Spec: `docs/blocks/AC-sidebar-group-state.md`, Prompt: `docs/blocks/AC-prompt.md`.

- **Phase A — Server-Read + Render.** `_sidebar_context.py`: defensiver Parser `_parse_open_group_ids` (`set[int]`, nur Ints, Garbage still verworfen, 512-Zeichen-/64-ID-Cap, niemals 500) + `_read_open_group_ids` (Guard `has_request_context()` → leeres Set ausserhalb Request-Kontext, hält die bestehenden context-less `build_sidebar_context()`-Unit-Tests grün). `sidebar_open_group_ids` im Context **beider** Render-Pfade (Context-Processor + Polling-Endpoint laufen durch `build_sidebar_context()` — single-source). Template `_group_section.html`: `open` + `aria-expanded` conditional, Undefined-Fallback `(sidebar_open_group_ids or ())` rendert collapsed.
- **Phase B — Client-Write.** `app/static/js/sidebar.js`: delegierter `toggle`-Listener in der Capture-Phase (Event bubbelt nicht), Filter auf `details.hostgroup` innerhalb `#server-list`, schreibt Cookie komplett neu aus dem DOM-Ist-Zustand (`Max-Age=31536000; Path=/; SameSite=Lax`, `Secure` über HTTPS), zieht `aria-expanded` am `<summary>` nach (Drive-by-Fix gegen hartes `false`). Keine JS-Test-Infra (Scope), keine neuen UI-Strings.
- **Phase C — Doku.** ARCHITECTURE.md §Group-Aufklapp-Zustand, CHANGELOG v0.18.0, ADR-0046 in `decisions/README.md` (war bereits vorhanden), STATE.md.

**Bewusst nicht im Block (ADR-0046 §Out of Scope):** Auto-Expand bei aktiver Sidebar-Suche (Re-Open-Trigger), geräteübergreifende DB-Persistenz, Aufklapp-Zustand anderer `<details>`-Flächen. **Operator-Browser-Smoke offen** (vom User abzuhaken): Gruppe auf → 60s-Poll → bleibt offen; Reload → bleibt offen; zweite Gruppe zu/auf; Browser-Neustart → bleibt offen; Suche schreibt kein Cookie. **Tag `v0.18.0`** erst nach Branch-Merge auf main setzen ([Tag-only-on-main-after-Merge]). **Nicht committen ohne explizite User-Anweisung** ([No-Auto-Commit]); die unrelated `docs/design/*`-Änderungen im Working-Tree gehören NICHT zu Block AC und sind beim Commit auszusparen.
...
- **Phase B — `c67e849`** — alle `flash()` + View-lokale Error-Strings (settings/findings/server_settings/servers/llm_settings/setup/auth) gemäß Glossar; `Bitte …`-Floskeln gestrichen. Audit-Event-Typen/Feldnamen/URLs unverändert.
- **Phase C — `2a84030`** — Settings-Surfaces (`settings/*` inkl. Nav „Groups", `servers/settings.html`, `_tag_editor.html`). „DSGVO" → „GDPR".
- **Phase D — `a4ce5bc`** — Server-Detail + Findings + `_partials/*` (größte Phase). OOB-Single-Source eingehalten, Drift-Test unverändert grün, kein `|safe` berührt. „— noch nie gescannt" → „— never scanned".
- **Phase E — `8f96654`** — Audit/Setup/Chat/Dashboard-Restdeutsch + `llm_prompt.py` System-Prompt (englisch; Marker `TRIVY_DATA_START/END` + Daten-Block + Finding-Line-Keys byte-identisch; Pass-2-Prompts unberührt) + `llm_chat.py` JSON-`message`-Werte (error-Codes unverändert).
- **Phase F — `d8d1f3b`** — 6 JS-Dateien (inkl. `stale.js`-Relative-Time-Mirror) + Sprach-Sweep-Test `tests/test_ui_language.py` (Pure-Unit, Default-Suite) + Sweep-Rest-Funde (settings.py-LLM-Reviewer-Flashes, findings.py-aborts, server_detail.py-Workflow-Labels, `_macros.html`-quick_copy, `_heartbeat_large.html`, `llm_provider.html`) + CHANGELOG v0.17.0.

**Bewusst nicht im Block (ADR-0045 §Scope):** Doc-Sprache/Code-Kommentare/Docstrings/ADRs bleiben deutsch; keine i18n; kein Daten-Rollout persistierter deutscher Strings (Audit-Metadata/Notes-Bestand). **Operator-Browser-Smoke offen** (vom User abzuhaken): Settings-Save-Flash, Bulk-Ack-Modal, Setup-Wizard-Step, Chat-Antwort englisch. **`sidebar.js` parallel von Block AB (Phase F: „kein Scan" → „no scan") und Block AC angefasst — Merge-Reihenfolge beachten.** **Tag `v0.17.0`** erst nach Branch-Merge auf main setzen (gemäß [Tag-only-on-main-after-Merge]).

**Aktiv 2026-05-29 — TICKET-008 Pass-2 Risk-Band Exploitability-Modell (ADR-0043).** Prompt-Rework verprobt (`eval/pass2/`, v2 gewählt). Fünf Blöcke (1 impact_class · 2 Render+Prompt+Schema · 3 action_type-Ableitung · 4 Tests+Evidence · 5 Daten-Rollout nur auf User-Anweisung). Treiber: Implementer-Subagenten. Provider seit 2026-05-29 DeepInfra statt Novita (gpt-oss-120b korrupt bei Novita).
