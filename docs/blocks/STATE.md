...
**Block AD abgeschlossen 2026-06-04 — Settings-Redesign: horizontale Tab-Nav + `s-*`-Schicht (ADR-0047).** Branch `feat/block-ad-settings-redesign` (von `main` abgezweigt, nachdem die Mockup-Design-Dateien `docs/design/*` als eigener Commit `eff172f` auf `main` lagen — User-Entscheidung). Zielversion v0.19.0. **Reines Restyling — keine Routen-/Schema-/Render-Helper-Änderung, keine Migration, kein neuer Endpoint.** `app/views/_settings_shell.py`, `settings.py`, `llm_settings.py` unangetastet (DoD #8 grün). Default-`pytest` **2253 passed / 201 skipped / 658 deselected** (~25 s; +36 neue Template-Tests, −5 obsolete Legacy-Smokes), `ruff`/`ruff format`/`mypy app/` grün, Frontend-Build grün (CSS-Bundle ohne `sd-manage`). Spec: `docs/blocks/AD-settings-redesign.md`, Prompt: `docs/blocks/AD-prompt.md`.
...
- **Phase B — Shell-Umbau (`59bb026`).** `_nav.html` vertikale `w-56`-Menu → `<nav class="settings-tabs">` mit 7 Anchor-Tabs (HTMX 1:1, Master-Key-Badge). `_shell.html`/`_page.html`: Tabs oben + `#settings-content.settings` (Frame bleibt über Tab-Swap stehen), `#detail-pane-content` unverändert. `_profile_dropdown.html`: Settings-active-Marker (Pfad-basiert).
