...
**Block AB geplant 2026-06-04 — English UI Migration (ADR-0045).** User-Entscheidung: gesamte UI nur noch englisch, in einem dedizierten Block (löst ADR-0033 §8 Phase-2-Strategie ab). Branch `feat/block-ab-english-ui`, Zielversion v0.17.0, **kein Markup-/Logik-Umbau, keine Migration** — reiner String-Touch in sechs Phasen (A Querschnitt/Filter/Forms · B Flash-Messages · C Settings · D Server-Detail+Findings · E Audit/Setup/Chat inkl. Chat-LLM-Prompt · F JS+Sprach-Sweep-Test). Spec: `docs/blocks/AB-english-ui-migration.md`. Doc-Sprache und Code-Kommentare bleiben deutsch. Umsetzung durch die Hauptsession, User reviewt den PR.
