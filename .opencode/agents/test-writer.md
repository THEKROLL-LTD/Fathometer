...
AGENTS.md "Documentation hygiene" is binding. For tests that means: **no ticket or ADR IDs in docstrings or comments** unless the test is incomprehensible without them. A test name states what is asserted; a docstring states why that matters. `tests/test_doc_budgets.py` enforces the length budgets on `CHANGELOG.md`, ADRs and `STATE.md` — treat it as a gate like any other.
