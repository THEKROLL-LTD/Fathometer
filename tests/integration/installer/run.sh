...
# `http://localhost:8000` (z.B. `docker compose up -d fathometer`).
#
...
BACKEND_URL="${FM_URL:-http://host.docker.internal:8000}"

build_and_run() {
  local distro="$1" dockerfile="$2"
  local image="fathometer-installer-test:${distro}"
  echo "==> building ${image}"
  ...
  docker run --rm \
    ...
    -e "FM_URL=${BACKEND_URL}" \
    -e "FM_UNATTENDED=1" \
    -e "FM_MASTER_KEY=${FM_MASTER_KEY:-test-master-key-32-bytes-minimum-entropy-aaa}" \
    -e "FM_SERVER_NAME=${distro}-installer-test" \
    -e "FM_INSTALL_TRIVY=yes" \
    "${image}"
}
