...
def test_agent_version_returns_json_shape(db_app: Flask) -> None:
    ...
    assert data["current_agent_version"] == "0.3.1"
    assert data["min_agent_version"] == "0.1.0"
    assert data["recommended_trivy_version"] == "0.70.0"
...
def test_agent_files_serves_whitelisted_scripts(db_app: Flask) -> None:
    """Task #6 — `GET /agent/files/secscan-agent.sh` liefert das Skript."""
    client = db_app.test_client()
    ...
    resp2 = client.get("/agent/files/secscan-register.sh")
    assert resp2.status_code == 200
    body2 = resp2.get_data(as_text=True)
    assert "secscan-register.sh" in body2
...
def test_install_sh_renders_with_backend_url(db_app: Flask) -> None:
    ...
    assert "0.70.0" in body  # RECOMMENDED_TRIVY_VERSION
    assert "0.3.1" in body  # CURRENT_AGENT_VERSION
