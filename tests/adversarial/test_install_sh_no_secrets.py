...
Patterns:
* `FM_MASTER_KEY=` mit einem Wert dahinter (kein Beispiel-Token).
* `Authorization: Bearer <hex/base64-token>`.
* `FM_API_KEY=<value>` (das schreibt der Wizard ZUR LAUFZEIT, nicht ins
  Template).
...
_FORBIDDEN_PATTERNS = [
    ...
    re.compile(r'FM_MASTER_KEY=["\'][A-Za-z0-9_\-+/=]{16,}', re.IGNORECASE),
    re.compile(r'FM_API_KEY=["\'][A-Za-z0-9_\-+/=]{16,}', re.IGNORECASE),
...
def test_install_sh_master_key_only_referenced_as_env_var_name(db_app: Flask) -> None:
    """`FM_MASTER_KEY` als Bash-Variablen-Name darf vorkommen (Unattended-
    ...
    # Aber NIE als `FM_MASTER_KEY="<token>"` mit konkretem Token-Wert.
    forbidden = re.compile(r'FM_MASTER_KEY\s*=\s*["\'][A-Za-z0-9_\-+/=]{8,}')
