"""Tests fuer ``app.services.llm_debug_log`` (v0.9.3, ADR-0023 §"(e)").

Drei Aspekte:

1. ``record()`` schreibt eine Row, applied Body-Cap, normalisiert Felder.
2. ``_apply_body_cap()`` schneidet Bodies >cap auf Stub-Dicts zurueck.
3. ``evict_old()`` macht Time-Cap (DELETE bei created_at < now - days) und
...
from app.services.llm_debug_log import _apply_body_cap, evict_old, record
