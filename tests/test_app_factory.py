...
- Settings werden geladen (Pflicht-Env-Var `FM_ENCRYPTION_KEY`).
...
def clean_env(monkeypatch: pytest.MonkeyPatch) -> Iterator[pytest.MonkeyPatch]:
    """Stellt sicher, dass keine `FM_*`-Vars aus dem Host-Env durchschlagen."""
...
def test_create_app_returns_flask_instance(clean_env: pytest.MonkeyPatch) -> None:
    clean_env.setenv("FM_ENCRYPTION_KEY", "x" * 32)
    clean_env.setenv("FM_DATABASE_URL", "postgresql+psycopg://test:test@127.0.0.1:1/test")
...
def test_max_body_mb_override_applies(clean_env: pytest.MonkeyPatch) -> None:
    """Override via `FM_MAX_BODY_MB` muss bis zu `MAX_CONTENT_LENGTH` durchschlagen."""
    clean_env.setenv("FM_ENCRYPTION_KEY", "x" * 32)
    clean_env.setenv("FM_DATABASE_URL", "postgresql+psycopg://test:test@127.0.0.1:1/test")
    clean_env.setenv("FM_MAX_BODY_MB", "5")
...
def test_missing_encryption_key_refuses_start(clean_env: pytest.MonkeyPatch) -> None:
    """Ohne `FM_ENCRYPTION_KEY` darf die App nicht starten."""
...
def test_too_short_encryption_key_refuses_start(clean_env: pytest.MonkeyPatch) -> None:
    ...
    clean_env.setenv("FM_ENCRYPTION_KEY", "tooshort")
    clean_env.setenv("FM_DATABASE_URL", "postgresql+psycopg://test:test@127.0.0.1:1/test")
