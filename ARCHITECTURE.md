...
**Settings-UI-Aufbau (Block AD / ADR-0047).** Die Settings-Fläche nutzt eine **horizontale Sticky-Tab-Navigation** (`.settings-tabs`) oben in `.main` — die globale Fleet-Sidebar bleibt links stehen. Sieben Tabs in fester Reihenfolge: Servers, Tags, Groups, LLM Provider, LLM Reviewer, Master-Key (Badge „new"), About. Tab-Klick swappt per HTMX nur `#settings-content` (`hx-target="#settings-content"`, `hx-swap="innerHTML"`, `hx-push-url`, `hx-headers='{"HX-Target": "settings-content"}'`), die Tab-Leiste bleibt stehen; `href`-Fallback für No-JS. Alle sieben Subseiten tragen die **`s-*`-Komponentenschicht** (`frontend/src/css/components/settings.css`, Token-only, kein DaisyUI/Tailwind) und den Header-Pattern `settings__eyebrow` (Text „Settings", ohne Nummerierung) + `settings__title` + `settings__lede`. Der **Drei-Modi-Render-Helper `render_settings()`** (ADR-0016: Vollseite / Detail-Pane-Fragment / Content-Fragment) und alle Routen/Schema bleiben unverändert — Block AD ist reines Restyling. Modals (Provider-Wechsel, Reviewer-Mode/-Requeue/-Concurrency) sind token-gestylte Alpine-`x-show`-Overlays statt DaisyUI; der Master-Key-Rotate-Confirm ist ein nativer `confirm()`-Dialog.
