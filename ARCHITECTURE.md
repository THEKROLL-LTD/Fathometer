...
- **`application_groups` / `application_group_evaluations`** — Application-Group-Gruppierung. Die Group hält fleet-weite Identität + Match-Patterns (`label`, `path_prefixes`, `pkg_name_*`, `pkg_purl_pattern`, `group_kind`, `source`). Die server-abhängige Bewertung (`risk_band`, `reason`, `worst_finding_id`, `action_type` …) liegt in der Junction. Seit ADR-0053 (Fix-Lane-Evaluation) ist der PK **dreispaltig** `(group_id, server_id, fix_lane)`; seit ADR-0061 (Fix-Ownership) gilt `fix_lane ∈ {patch, upstream, mitigate}` — bis zu **drei** Eval-Rows pro `(group, server)`, eine je Fix-Lane. `fix_lane` ist deterministisch aus `Finding.fixed_version` **und `Finding.finding_class`** abgeleitet (Single-Source `risk_engine.fix_lane_for` + spiegelnder SQL-`case`): `patch` = host-applizierbarer Fix (`os-pkgs` mit `fixed_version`), `upstream` = Fix existiert, ist aber **nicht** host-applizierbar (`lang-pkgs`/`other` mit `fixed_version` — z. B. eine in ein gobinary einkompilierte stdlib-Version, die nur ein Upstream-Rebuild schließt, kein `dnf/apt upgrade`), `mitigate` = kein Fix. Seit ADR-0062 (Block AH) verfeinert ein optionaler, vom Host-Agent gemeldeter Flag `Finding.host_update_available` die Ableitung: ein lang-pkgs-Finding, dessen besitzendes OS-Paket laut `dnf/apt` ein Repo-Update hat (`true`), wird von `upstream` nach `patch` promotet; `false`/`NULL` bleibt `upstream`. Die Lane selbst ist keine persistierte Finding-Spalte und kein LLM-Output (`host_update_available`/`owning_package`/`available_version` dagegen sind persistierte, nullable Finding-Spalten). So bewerten mehrere Server denselben Group-Pattern ohne last-write-wins-Konflikt, und der patchbare vs. nicht-patchbare Teil einer Gruppe kann unterschiedliche Bands tragen. Findings erben ihren Band aus der für ihren Server **und ihre Lane** zuständigen Junction-Row.
...
**Risk-Reviewer (Worker, Two-Pass).** Die deterministische Pre-Triage (§12) liefert `pending`-Findings als Eingabe. **Pass 1 (Group-Detection)** erzeugt aus ungrouperten Findings wiederverwendbare `application_groups` mit Match-Patterns — Eingabe nur Finding-Identität, kein Server-Kontext. **Pass 2 (Risk-Evaluation)** bewertet seit ADR-0053 **pro Fix-Lane** statt pro Gruppe: eine gemischte Group erzeugt bis zu **drei getrennte LLM-Requests** (je einen für die `patch`-, `upstream`- und `mitigate`-Lane, ADR-0061), mit identischem Host-/Exposure-Kontext-Block und vollem Selektions-Budget pro Lane. Jeder Call liefert genau ein Verdikt — `risk_band` plus `worst_finding_id` und `reason` (max 256 chars); `pending`/`unknown` sind als LLM-Output verboten. Der **`action_type` wird nicht mehr vom LLM emittiert**, sondern deterministisch aus `(fix_lane, risk_band)` abgeleitet und persistiert (`patch+escalate/act → patch`, `patch+monitor → watch`, `patch+noise → none`, `mitigate/upstream+escalate → mitigate`, `mitigate/upstream+monitor → watch`, `mitigate/upstream+noise → none`). Band und Lane sind nicht orthogonal: **`act` ist patch-only** (ein nicht host-applizierbar fixbares Finding ist entweder `escalate` oder `monitor`/`noise`, nie `act`) — Band-Whitelist `patch: {escalate, act, monitor, noise}`, `mitigate`/`upstream`: `{escalate, monitor, noise}`, durchgesetzt im jeweiligen Lane-Prompt und im Validator. Die `upstream`-Lane macht die ehrliche Aussage: ein Fix existiert (Version bleibt sichtbar), ist aber nicht per Paketmanager applizierbar — anders absichern bis zum Vendor-Rebuild, keine „Apply app update"-Empfehlung. Pass 2 setzt `risk_band_source='llm'`; die Pre-Triage überschreibt diese Findings beim Re-Ingest nicht.
