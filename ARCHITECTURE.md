# fathometer — Architektur & Implementierungs-Plan
...
Die Lücke dazwischen — ein dauerhaft laufendes, einfaches Dashboard für die OS-Pakete laufender Root-Server, mit Triage-Workflow und Audit-Historie, in einem Container-Compose self-hostbar — füllt fathometer.

### Sicherheits-Stance: Push statt Pull, keine Server-Credentials

Eine zentrale Architektur-Entscheidung, die jede spätere Funktion mitprägt: **die App hat keinen Zugriff auf die Server, die sie überwacht.** Server pushen ihre Trivy-Scans aktiv an den fathometer-Server (über Cron oder einen systemd-Timer); fathometer selbst initiiert keine Verbindung zu den Root-Servern. Es gibt im gesamten System keine SSH-Keys, keine Server-Passwörter, keine Sudo-Credentials und keine Inbound-Verbindungen vom fathometer-Server zur überwachten Flotte.
...
Bei fathometer gilt stattdessen: wer den fathometer-Server kompromittiert, sieht die Schwachstellen-Liste der überwachten Server. Das ist unangenehm, weil es einem Angreifer Hinweise auf lohnende Ziele gibt, aber es verschafft ihm keinen direkten Zugang zu den Servern selbst — er muss die jeweilige Schwachstelle separat ausnutzen, was zusätzliche Hürden bedeutet. Der einzige geheime Wert auf der fathometer-Seite ist der **Master-Key**, mit dem Server sich registrieren und einen eigenen Server-Key aushandeln. Master-Key und Server-Keys sind jederzeit unabhängig voneinander rotierbar; ein Leak des Master-Keys erlaubt nur die Registrierung neuer (Phantom-)Server, was im Audit-Log sofort auffällt — und schadet den echten Servern nicht.

Diese Stance ist auch der Grund, warum Notifications bewusst nicht im MVP enthalten sind: jeder Notification-Channel (SMTP-Credentials, Discord-Webhook, Slack-Bot-Token) ist ein zusätzlicher geheimer Wert auf dem fathometer-Server, der bei einem Kompromiss zusätzlich leakt. Wir bauen das später ein, aber durchdacht und mit der Möglichkeit, sensitive Credentials in einen externen Secret-Manager auszulagern.
...
```
   ...
   └─────────────────┬───────────────────┘
                     ...
                     ▼
   ┌─────────────────────────────────────┐    ┌──────────────┐
   │      fathometer App-Container          │───▶│  DeepInfra   │
...
**Block R (ADR-0026, ADR-0042) — asynchroner Scan-Ingest (einziger Pfad seit v0.12.0).** `POST /api/scans` antwortet binnen <1s mit `202 Accepted` und einem Body `{job_id, status:"queued"}`. Die volle Verarbeitung (Pydantic-Parse, Findings-UPSERT, Host-State-Persist, Pre-Triage-Loop, Group-Matching, Pass-1-/Pass-2-Job-Queueing, `notify_conversations_for_scan`) lebt im `fathometer-llm-worker` als Sub-Tick `scan_ingest_tick` (vor LLM-Pickup im `_tick()` priorisiert). Edge-Schritte: Bearer-Auth → Server-Active → Rate-Limit → gzip-Decompress → SHA-256-Hash über den dekomprimierten Body → schmale Pre-Validation (Top-Level-Objekt, `agent_version`, `host.hostname`, `scan`) → Agent-Version-Gate → Per-Server-Soft-Cap (Default 50 queued/in_progress, ENV `FM_MAX_QUEUED_INGEST_JOBS`) → `INSERT INTO scan_ingest_jobs` mit `ON CONFLICT DO NOTHING` auf dem partial-unique `payload_sha256`-Index (`status IN ('queued','in_progress')`). Idempotenter Re-Upload des gleichen gzipped Bodies retourniert dieselbe `job_id` ohne zweiten `scan.queued`-Audit-Event. **Der Agent beendet nach der 202-Annahme sofort (Fire-and-Forget, ADR-0042) — kein Polling, kein Job-Status-Endpoint.** Scan-Fortschritt und -Ergebnis (inkl. `failed` mit Audit `scan.ingest_failed`) sind serverseitig über die `scan_ingest_jobs`-Zeile und das Dashboard-Polling (ADR-0019) sichtbar. Der gzipped Payload wird _atomar im selben UPDATE_ wie der Status-Wechsel auf `done` auf `NULL` gesetzt (ADR-0005-Transit-Ausnahme); bei `failed` bleibt der Payload 24 h für Operator-Debug erhalten, danach Retention-DELETE. Das ursprünglich in Block R Phase H als Cutover-Schutz eingeführte Feature-Flag `FM_SCAN_INGEST_ASYNC` ist mit v0.12.0 ersatzlos entfernt — Async ist der einzige Pfad.
...
- `POST /findings/bulk-acknowledge` (Body: `{finding_ids?, match?, server_scope?, comment?}`) — Bulk-Acknowledge in drei Flavors (genau einer muss befüllt sein). **Flavor A** mit `finding_ids` (explizite Liste): wirkt auf genau die übergebenen IDs (verwendet vom Checkbox-Auswahl-Flow im Server-Detail). **Flavor B** mit `match` (`cve_id` und/oder `package_name`, optional Tag/Status): wirkt auf *alle* matchenden offenen Findings über die gesamte Flotte, ungeachtet aktueller Filter (verwendet vom "Alle abhaken über alle Server"-Knopf in der globalen Suche). **Flavor C** mit `server_scope` (`{server_id, risk_band}`, ADR-0044): der Server resolved selbst *alle* offenen Findings dieses Servers in diesem Band — kein ID-Transport durch den Client, kein Limit. `risk_band` ist eine Whitelist `escalate/act/mitigate/monitor/noise`; `pending`/`unknown` werden mit 422 abgelehnt. `comment` ist optional; falls vorhanden, wird er pro betroffenem Finding als Notiz angelegt (ein Bulk-Insert). Audit-Event mit den betroffenen Finding-IDs (auf 50 gecappt) und vollem `count` in `metadata`. Server liefert bei `dry_run: true` eine Vorschau-Antwort — für Flavor A/B `{count, server_count, finding_ids}`, für Flavor C `{count, examples (max 5), server_scope}` (kein `finding_ids`-Transport) — das Frontend nutzt das für die Modal-Anzeige.
...
Die UI bleibt bewusst flach. Die obere Nav hat zwei sichtbare Items (**Dashboard**, **Findings** — getrennt seit ADR-0025; Suche-Eingang führt jetzt auf `Findings?q=…`) und ein Profile-Dropdown mit Settings/Audit/Logout (ADR-0016 für den Header-Aufbau). Das Theme ist statisch Dark — kein Toggle (ADR-0031).
...
Darunter eine **einzelne Findings-Ansicht** — die mit ADR-0018 eingeführten Modi `gruppiert nach Paket` und `diff seit letztem Scan` sind mit ADR-0025 ersatzlos entfallen, weil Application-Groups (ADR-0023) die Bündelung semantisch sauberer abbilden. Veraltete `?mode=group`/`?mode=diff`-URLs werden still ignoriert und rendern die Standard-Ansicht. Die Ansicht zeigt Filter-Chips (Severity, Status, **Class-Toggle** OS-Pakete/Library-Findings/beide, `Fix verfügbar`, `nur KEV`, EPSS-Range, Such-Input). Standardmäßig werden Findings primär nach **`application_group`** gruppiert (ADR-0023). Mit **ADR-0041 (Block AA)** ist der frühere Flat-Switch (`?flat=1` + flache Tabelle bei aktivem Filter) ersatzlos entfernt — die Group-Card-/Risk-Band-Ansicht ist der einzige Render-Pfad; veraltete `?flat=`-URLs werden still ignoriert. URL-Filter narrowen die Server-Detail-Ansicht damit nicht mehr (es gibt dort keine Filter-Bar-UI; dokumentierter Re-Open-Trigger in ADR-0041). Default-Sort: KEV zuerst, dann EPSS desc, dann CVSS desc. Resolved-Findings tragen eine grüne Pille mit Resolved-Datum, sonst gleiche Spalten wie offene. Jede Zeile zeigt Paket+Version, CVE-ID (klickbar Mitre/NVD-Link, mit kleinem **Copy-to-Clipboard-Icon** rechts), Severity-Pill, **CVSS-Score** in numerischer Form (z.B. `8.7`), **EPSS-Badge**, **KEV-Badge** wenn aktiv, Fix-Verfügbarkeit-Indikator. Eine Checkbox-Spalte erlaubt Auswahl; eine Action-Bar erscheint sobald etwas ausgewählt ist mit „Auswahl abhaken"-Knopf (Bulk-Acknowledge mit Bestätigungs-Modal). **Klick auf eine Zeile klappt einen Inline-Body auf** (ADR-0041) mit KI-Bewertung (`risk_band_reason`), voller CVE-Beschreibung, Primary-URL, Reference-Liste und Notizen-Thread; „Abhaken …"/„Re-open …" öffnet das beibehaltene Ack-/Reopen-Modal. `primary_url` wird seit Block AA persistiert (`findings.primary_url`).
...
**`/settings`** enthält die globalen Einstellungen: Severity-Schwelle, Stale-Threshold, Stale-DB-Threshold, **LLM-Provider-Block** (Preset-Dropdown mit DeepInfra/OpenAI/Together/Groq/Ollama/Custom — füllt `base_url` mit dem passenden Endpunkt vor; freie Felder für Provider-Anzeigename, Base-URL, API-Key, Modell-Name, Tages-Token-Cap; "Verbindung testen"-Knopf der eine 1-Token-Anfrage gegen den konfigurierten Endpunkt macht und Latenz + erfolgreiche Authentifizierung zurückmeldet), Master-Key rotieren (mit Bestätigung), Tag-Verwaltung (Liste mit Erstellen/Löschen/Färben) und die Liste der registrierten Server mit Revoke- und Retire-Knöpfen.

**Settings-UI-Aufbau (Block AD / ADR-0047).** Die Settings-Fläche nutzt eine **horizontale Sticky-Tab-Navigation** (`.settings-tabs`) oben in `.main` — die globale Fleet-Sidebar bleibt links stehen. Sieben Tabs in fester Reihenfolge: Servers, Tags, Groups, LLM Provider, LLM Reviewer, Master-Key, About. Tab-Klick swappt per HTMX nur `#settings-content` (`hx-target="#settings-content"`, `hx-swap="innerHTML"`, `hx-push-url`, `hx-headers='{"HX-Target": "settings-content"}'`), die Tab-Leiste bleibt stehen; `href`-Fallback für No-JS. Da die Tab-Leiste **außerhalb** des Swap-Targets liegt, zieht `app/static/js/settings_tabs.js` den Active-Marker (cyan Bottom-Border) client-seitig pfad-basiert nach (`htmx:pushedIntoHistory` + `popstate`). Alle sieben Subseiten tragen die **`s-*`-Komponentenschicht** (`frontend/src/css/components/settings.css`, Token-only, kein DaisyUI/Tailwind) und den Header-Pattern `settings__eyebrow` (Text „Settings", ohne Nummerierung) + `settings__title` + `settings__lede`. Der **Drei-Modi-Render-Helper `render_settings()`** (ADR-0016: Vollseite / Detail-Pane-Fragment / Content-Fragment) und alle Routen/Schema bleiben unverändert — Block AD ist reines Restyling. Die **External-Feeds-Freshness** (EPSS/CISA-KEV, read-only) liegt auf dem **About**-Tab. Modals (Provider-Wechsel, Reviewer-Mode/-Requeue/-Concurrency) sind token-gestylte Alpine-`x-show`-Overlays statt DaisyUI und werden per `x-teleport="body"` an `<body>` gehängt (sonst fängt das `container-type:inline-size` von `.settings` das `position:fixed`-Overlay als Containing-Block ab); der Master-Key-Rotate-Confirm ist ein nativer `confirm()`-Dialog.
...
- **Keine Server registriert**: "Noch kein Server in der Flotte. Master-Key in Settings → API-Keys generieren, dann auf dem Ziel-Server `fathometer-register.sh` ausführen. Anleitung im [Agent-README](agent/README.md)."
...
- **Per-Band „Acknowledge all"-Hover-Control (ADR-0044).** Jede Risk-Band-Sektion der Triage-Queue **außer `pending`** trägt im `<summary>`-Header ein bei Hover sichtbares „Acknowledge all"-Control (grau → cyan bei direktem Hover). Klick öffnet ein generisches Modal mit Band-Count und max. 5 Beispiel-Findings aus der `dry_run`-Antwort (plus „… and N more"), Pflicht-Bestätigung, dann Aufruf von `POST /findings/bulk-acknowledge` mit dem **Flavor C** `server_scope={server_id, risk_band}`. Der Server resolved alle offenen Findings dieses Bands selbst (kein ID-Transport, kein 50er-Limit). `pending` ist server-seitig über die `risk_band`-Whitelist verboten (422), `unknown` hat keine eigene Sektion. Der frühere noise-only-Toolbar-Link, das Noise-Fragment (`GET /<id>/fragments/noise`) und `risk_band_filter` sind mit ADR-0044 entfallen.

### Was Block I bewusst NICHT macht

- Kein Dark-Mode-Default-Wechsel. *(Nachtrag: Theme-Toggle mit ADR-0031 vollständig entfernt — Theme ist statisch dark.)*
...
**Body-Size-Limit vor JSON-Parsing.** Flask's `MAX_CONTENT_LENGTH` lehnt Bodies oberhalb einer Schwelle ab, *bevor* der Body überhaupt gelesen oder geparst wird. Default für `/api/scans` ist 64 MB (das ist die Wire-Größe; Trivy-Scans komprimieren typisch 7–10×, ein 64-MB-gzipped-Body entspricht mehreren hundert MB Roh-JSON — die 10-MB-Annahme war für sehr große Hosts mit vielen tausend Findings zu knapp, beobachtet 2026-05-28: 20 MB gzip / 137 MB Roh-JSON → 413), für `/api/register` und `/login` 4 KB. Die Schwelle ist über die Environment-Variable `FM_MAX_BODY_MB` konfigurierbar. Beantwortet wird mit `413 Payload Too Large` und einer klaren Fehlermeldung.

**Gzip-Bomb-Schutz.** Da `/api/scans` `Content-Encoding: gzip` akzeptiert, ist der theoretische Worst-Case ein 10-MB-Body der zu mehreren GB dekomprimiert (klassische Zip-Bomb mit hochrepetitiven Daten kann Faktor 1000+ erreichen). Das würde Worker-RAM und CPU sprengen. Lösung: **Streaming-Decompress** mit hartem Decompress-Bound. Der Decompress läuft chunk-weise (z.B. 64 KB Buffer) durch `zlib.decompressobj()` mit einem mitlaufenden Bytes-Zähler; sobald `decompressed_size > FM_MAX_DECOMPRESSED_MB` (Default 512 MB), wird die Verarbeitung abgebrochen und mit `413 Payload Too Large — Decompressed-Limit überschritten` beantwortet. Das schützt vor Bombs ohne legitime große Scans (137 MB Roh = ~20 MB gzipped, geht durch) zu blockieren. Tradeoff: ein höherer Bound bedeutet mehr Worker-RAM pro Request im Worst-Case (durch den Streaming-Bound auf den Bound-Wert gedeckelt) — bei knappem RAM `FM_MAX_DECOMPRESSED_MB`/`FM_MAX_BODY_MB` heruntersetzen. Der Decompress läuft zwingend vor dem Pydantic-Parse — niemals erst alles dekomprimieren und dann parsen.
...
**Reference-Agent: `agent/fathometer-agent.sh`.** Setzt voraus, dass `trivy` (>= 0.70.0 — Mindestversion für vollständige EPSS-/KEV-/Attack-Vector-Felder im JSON), `curl`, `jq` und `gzip` auf dem Host installiert sind und mit root-Rechten läuft (damit `trivy rootfs /` alle Pakete sieht — sowohl OS-Pakete als auch eingebettete Library-Findings in installierten Binaries). Liest `FM_URL` und `FM_API_KEY` aus der Umgebung. Sammelt die Host-Info aus `/etc/os-release` und `uname`, ruft `trivy rootfs / --format json --scanners vuln` auf (im MVP nur Vulnerability-Scanner — Secret- und Misconfig-Scanner sind out of scope und werden nicht aktiviert; `rootfs` statt `fs`, weil `fs` nur OS-Pakete erfasst und Go-/Java-Binaries unter `/usr/local/bin`, `/var/lib/...` überspringt), baut den Envelope per `jq`, **komprimiert das Ergebnis mit `gzip`** und sendet es per `curl` als `POST /api/scans` mit `Authorization: Bearer ${FM_API_KEY}` und `Content-Encoding: gzip`. Gemessen am Beispiel-Scan: 4.95 MB JSON werden zu 0.56 MB on the wire (8.9×). **Trivy-DB-Frische** muss der Agent nicht separat sammeln — Trivy schreibt `Metadata.DataSource` und `Metadata.UpdatedAt` selbst in den Report, der Server extrahiert daraus `trivy_db_version` und `trivy_db_updated_at`. **EPSS- und KEV-Daten** liefert Trivy ebenfalls direkt im Vulnerability-Block (`PublishedDate`, `LastModifiedDate`, `CVSS`, `VendorSeverity`, plus die `CISAKnownExploitedVulnerabilities`- und `EPSS`-Felder wenn die Trivy-DB sie kennt) — keine zusätzliche Anreicherung nötig. Exit-Codes: 0 OK, 1 fehlende Voraussetzungen, 2 Trivy-Fehler, 3 Upload-Fehler — damit eine Cron-Mail oder Monitoring-Integration unterscheiden kann.

**Register-Helper: `agent/fathometer-register.sh`.** Wer einen Master-Key hat, kann damit einen neuen Server registrieren und bekommt den Server-Key auf stdout zurückgegeben — geeignet zum Pipen in eine Key-File oder Secret-Manager. Master-Key kommt aus `FM_MASTER_KEY` env oder wird interaktiv abgefragt (silent read, keine History). Aufruf: `./fathometer-register.sh <server-url> <server-name> [interval-h]`.
...
(Alternative, falls stdin als TTY erhalten bleiben soll: `sudo bash <(curl -fsSL https://fathometer.example.com/install.sh)`.) Das Backend liefert den Installer als Jinja-gerendertes Bash-Skript aus; er ist ein interaktiver Wizard mit englischsprachiger TTY-Ausgabe (Box-Borders, ANSI-Farben, Status-Symbole `[ok] / [..] / [fail]`) und führt durch sechs sichtbare Phasen: (1) **System detection** (`/etc/os-release`, `uname -m`, `command -v systemctl`), (2) **Dependencies** (`curl`/`jq`/`gzip` pro Distro via `apt-get`/`dnf`/`yum`/`zypper`), (3) **Trivy** (Pin-Install nach `/opt/fathometer/bin/trivy` mit SHA256-Verifikation, falls fehlend oder zu alt), (4) **Server registration** (interaktive Eingabe von Server-Name, Scan-Intervall und Master-Key via `read -srp < /dev/tty`; API-Key wird nach `/etc/fathometer/agent.env` mit `chmod 0600 root:root` geschrieben), (5) **Scheduler** (systemd-Unit + Timer mit `RandomizedDelaySec=2h`; Cron-Fallback falls kein systemd), (6) **Probe scan** (synchroner Run des Agents mit Live-stderr, der Operator sieht im selben Terminal das `HTTP 202` der ersten Envelope-Upload). Master-Key wird nie via Argv oder ENV mitgegeben — ausschließlich im Wizard-Prompt. Nicht-interaktiver Modus für Ansible/Cloud-Init via `FM_UNATTENDED=1` plus `FM_MASTER_KEY`/`FM_SERVER_NAME`. Details und Sicherheits-Überlegungen siehe ADR-0021.

**Power-User-Pfad (Alternative).** Wer Ansible/Salt/Puppet einsetzt oder den Wizard ablehnt, kann weiterhin die zwei Skripte direkt vom Repo (oder via `GET /agent/files/<name>`) ziehen, `fathometer-register.sh` aufrufen, den zurückgegebenen Server-Key in `/etc/fathometer/agent.env` schreiben und den Agent selbst in cron oder als systemd-Timer einhängen. Die Skripte sind absichtlich klein gehalten — der Operator soll sie vor dem Ausführen lesen können.
...
- `GET /install.sh` — rendert das Jinja-Template `app/templates/agent/install.sh.j2` mit den eingebackenen Konstanten `FM_URL` und `RECOMMENDED_TRIVY_VERSION`. Content-Type `text/x-shellscript`.
- `GET /agent/files/<name>` — liefert `fathometer-agent.sh` und `fathometer-register.sh` als statische Files (`send_from_directory` mit harter Whitelist; alles andere → 404, inklusive Pfad-Traversal-Versuche).
...
**Agent v0.2.0-Erweiterungen.** Das Skript `agent/fathometer-agent.sh` schreibt zusätzlich `host.trivy_version` ins Envelope (kleiner `trivy --version | awk`-Helper) und strippt vor dem Envelope-Build den Block `Results[].Packages[]` per `jq 'del(.Results[].Packages)'` aus dem Trivy-Output. Erwarteter Win: raw 4.95 MB → 400-700 KB (80-90% Reduktion); gzipped 560 KB → 100-200 KB. Begründung: die Packages-Inventarliste (`InstalledFiles`, `Maintainer`, `DependsOn`, `Licenses`) ist explizit out-of-scope nach §17 (SBOM/License-Findings) und wird im Ingest ohnehin per `extra="ignore"` verworfen — der Agent-side-Strip vermeidet den Bandbreiten- und Pydantic-Walk-Overhead. Fallback auf ungestripped bei `jq`-Fehler. **Wichtig:** Trivy schreibt `PkgIdentifier`/`SeveritySource`/`VendorIDs` zusätzlich in jeden Vulnerability-Eintrag, sodass der Strip die Ursachen-Felder (siehe nächster Absatz und §6) nicht entwertet.
...
**Ursachen-Felder pro Finding (ab v0.7.0).** Aus jedem Trivy-Vulnerability-Block werden fünf zusätzliche Werte persistiert (`package_purl`, `target_path`, `result_type`, `severity_source`, `vendor_ids` — siehe §6 für die Schema-Details). Diese Felder wurden bis Block AA als **Ursachen-Sub-Zeile** in der flachen Findings-Tabelle gerendert. Mit **ADR-0041 (Block AA)** ist die flache Tabelle entfernt und der neue Inline-Body zeigt diese Felder bewusst nicht (Less-is-more); die Daten bleiben **persistiert** (Ingest + Model unverändert) und sind ein dokumentierter Re-Open-Trigger für eine spätere Inline-Anzeige. Der ursprüngliche Anzeige-Vertrag war: bei OS-Pkg-Distros (`ubuntu`, `debian`, `rhel`, `alpine`, ...) `{package_name} {installed_version} ({result_type})` plus VendorID-Pills, bei Lang-Pkgs (`gobinary`, `jar`, `npm`, `pip`, ...) `{package_name} {installed_version} in {target_path}`. Damit sieht der Operator auf einen Blick, ob es ein Distro-Paket (Update via Paketmanager) oder eine eingebettete Library (Update via App-Rebuild/Redeploy) ist — **ohne** dass das Backend einen konkreten Update-Befehl vorschlägt. Bewusst kein statisches `UPDATE_TEMPLATES`-Mapping: Distro-Familie alleine ist nicht genug Kontext (Snap/Flatpak/k3s/Container-Hosts/embedded `gobinary` brechen ein generisches Template). LLM-basierte Fix-Empfehlung als Re-Open-Trigger in ADR-0021 dokumentiert, eigene ADR nötig — siehe §17.

**Was der Agent NICHT macht.** Keine Auto-Updates des Agents (sonst Supply-Chain-Risiko — das Backend zu kompromittieren würde Root auf der gesamten Flotte bedeuten), kein Datei-Versand außer dem Scan-Envelope, kein Lauschen auf Inbound-Verbindungen, kein Schreiben in Verzeichnisse außerhalb von `/tmp` und `/opt/fathometer/`. Der Agent ist ein Push-Only-Cron-Job, kein Daemon. Update-Pfad ist derselbe Einzeiler wie der Install-Pfad (`curl … | sudo bash`) — der Installer erkennt ein bestehendes `agent.env` und überspringt Phase 4 (Server registration).
