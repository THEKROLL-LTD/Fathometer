...
- **`group_chat_conversations` / `group_chat_messages`** — Per-Group-AI-Chat (Block AE, ADR-0055, Migration `0023`). Genau **eine** Konversation pro `(server_id, application_group_id)` (`UNIQUE`-Constraint `uq_group_chat_conversations_server_group`); beide FKs `ON DELETE CASCADE`. `server_id` ist `Integer` (`servers.id`), `application_group_id` ist **`BigInteger`** (`application_groups.id` ist BigInteger — analog `application_group_evaluations`/`llm_debug_log`). `model` hält `Setting.llm_model` zum Snapshot-Zeitpunkt; `findings_snapshot_at` nur den Einfrier-Zeitpunkt (Debug/Audit). Es gibt **kein Findings-Bridge-Table** — der eingefrorene Findings-Snapshot lebt im persistierten System-Prompt (erste Message, `role=system`). `group_chat_messages` trägt `role` als nativen Enum `chat_message_role` (`system`/`user`/`assistant`), `content` (Text), optionale `prompt_tokens`/`completion_tokens` und einen Lookup-Index `(conversation_id, created_at, id)`. „New Chat" löscht die Konversation (CASCADE auf die Messages); die nächste Nachricht legt sie frisch an.
