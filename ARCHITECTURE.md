...
Die alte Server-Karten-Grid-Variante des Dashboards (Block D/I) ist mit ADR-0017 in die Sidebar-Server-Liste gewandert; mit ADR-0020 fällt zusätzlich die separate „Aufmerksamkeit nötig"-Sektion weg (KEV-/Critical-/Stale-Quick-Filter sind jetzt die KPI-Cards), ebenso die alte separate Filter-Bar (`Anwenden`-Button), die globale Suche-View (`/findings/search`) und der dashed-border-Platzhalter.
...
- *Liste* (Default) — Findings-Tabelle mit Filter-Chips: Severity, Status (open/ack/resolved als Multi-Select-Chip-Gruppe; Default-Filter blendet resolved aus, ein Klick auf den `resolved`-Chip schaltet sie dazu), **Class-Toggle** (`OS-Pakete` / `Library-Findings` / beide; Default zeigt OS-Pakete vorn und Library-Findings darunter, ein Click filtert weg), `Fix verfügbar`-Toggle, `nur KEV`-Toggle, EPSS-Range-Slider, Such-Input für Paket/CVE. Resolved-Findings tragen eine grüne Pille mit Resolved-Datum, sonst gleiche Spalten wie offene. Jede Zeile zeigt Paket+Version, CVE-ID (klickbar Mitre/NVD-Link, mit kleinem **Copy-to-Clipboard-Icon** rechts), Severity-Pill, **CVSS-Score** in numerischer Form (z.B. `8.7`), **EPSS-Badge** (Prozentwert + Farb-Codierung), **KEV-Badge** wenn aktiv, Fix-Verfügbarkeit-Indikator, Erste-Gesehen-Datum. Default-Sort: KEV zuerst, dann EPSS desc, dann CVSS desc. Eine Checkbox-Spalte erlaubt Auswahl, eine Action-Bar erscheint unten wenn etwas ausgewählt ist mit "Auswahl abhaken"-Knopf (Bulk-Acknowledge mit Bestätigungs-Modal "X Findings abhaken? Optional Kommentar dazu"). Klick auf eine Zeile öffnet das Finding-Detail-Modal.
  ...
  **Group-Layer (Block P, ADR-0023):** Im Liste-Modus werden die Findings zusätzlich primär nach **`application_group`** gruppiert. Pro Application-Group eine **Group-Card** mit Group-Label (z.B. „k3s"), Finding-Count, Risk-Band-Pill (Group-Level-Band aus LLM-Pass-2 mit Worst-Case-Logik über alle Group-Findings), `risk_band_reason` als Mono-Font-Block und Worst-Finding als hervorgehobener Eintrag unter dem Card-Header. Cards mit Band `escalate`/`act`/`mitigate` sind default-expanded, Cards mit `monitor`/`noise` default-collapsed. Solange Pass-2 für eine Group noch nicht durch ist (Group existiert, `risk_band IS NULL`), wird die Card als grauer **`evaluating`-State** mit Spinner und Text „Evaluating risk for N findings, this may take a few minutes..." gerendert; das Dashboard-Polling löst den State graduell auf. Findings ohne `application_group_id` (Pass 1 noch ausstehend oder Pattern-Match negativ) laufen am Tabellen-Ende in einer **„Pending grouping"**-Sektion als flache Liste — verschwindet sobald Pass 1 sie greift. Das Block-K-Tabellen-Markup wird innerhalb der Group-Cards wiederverwendet (Spalten unverändert).
...
Eine eigene Such-View `/findings/search` gibt es seit ADR-0020 nicht mehr — die globale Volltext-Suche (CVE-ID/Paket/Server-Name) ist in die Dashboard-Findings-Tabelle gewandert (`?q=…`). Bulk-Acknowledge über mehrere Server ist weiterhin verfügbar: Tabelle filtert auf z. B. `?q=CVE-2024-6387`, Auswahl per Checkbox, Bulk-Ack-Modal listet die ausgewählten Findings inkl. Server-Namen, Submit ackt sie cross-server.
